...
describe("\n Happy path - Upload valid mock invoice and verify data is seen in the billing view\n", () => {
  ...
  test.each`
    testCase           | itemDescriptions                       | expectedServices                       | expectedEvents                                              | vendorId                       | contractName                                 | invoiceIsQuarterly
  ...
  `(
    ...
    async (data) => {
      const lineItemArrays = data.itemDescriptions.map((description: string) =>
        randomLineItems(1, { description })
      );
      const invoiceData = randomInvoiceData({
        date: new Date("2023-03-31"),
        lineItems: lineItemArrays.flat(),
        vendor: {
          id: data.vendorId,
        ...
        isQuarterly: data.invoiceIsQuarterly,
      });
      ...
      const expectedMonth = invoice.isQuarterly
        ? getQuarterMonthFromDate(invoice.date)
        : invoice.date.getMonth() + 1;
      const queryString = `SELECT * FROM "btm_billing_curated" where vendor_id = '${
        data.vendorId
      }' and month = '${padZero(expectedMonth)}'`;
      const queryObjects = await queryAthena<BillingCurated>(queryString);
      expect(queryObjects.length).toEqual(data.itemDescriptions.length);
      ...
      for (let i = 0; i < queryObjects.length; i++) {
        ...
        expect(queryObjects[i].invoice_is_quarterly).toEqual(
          invoice.isQuarterly.toString()
        );
      }
  ...
  test.each`
    testCase           | invoiceDate     | itemDescriptions                                    | vendorId                       | expectedEvents                                              | expectedServices                       | expectedMonthString | expectedContractId                           | invoiceIsQuarterly
    ${"non-quarterly"} | ${"2023-03-31"} | ${["Fraud check", "Passport check", "Long weight"]} | ${VendorId.vendor_testvendor1} | ${[EventName.VENDOR_1_EVENT_3, EventName.VENDOR_1_EVENT_1]} | ${["Fraud check", "Passport check"]}   | ${"03"}             | ${ContractName.vendor_testvendor1_contract1} | ${false}
    ${"quarterly"}     | ${"2023-06-30"} | ${["Six Data Validation Application"]}              | ${VendorId.vendor_testvendor6} | ${[EventName.VENDOR_6_EVENT_1]}                             | ${["Six Data Validation Application"]} | ${"04"}             | ${ContractName.vendor_testvendor6_contract1} | ${true}
  `(
