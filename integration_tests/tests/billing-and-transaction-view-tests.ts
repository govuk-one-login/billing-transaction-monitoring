...
describe("\nUpload pdf invoice to raw invoice bucket and verify BillingAndTransactionsCuratedView results matches with expected data \n", () => {
  test.each`
    ...
    ${"BillingQty equals TransactionQty for quarterly invoice with different month but same quarter as events"} | ${"VENDOR_6_EVENT_1"} | ${"Six Data Validation Application"} | ${"vendor_testvendor6"} | ${"2005/08/31 10:00"} | ${"2005/09/30"} | ${"0.32"} | ${2}                | ${"0.0"}                  | ${"£0.64"}            | ${"2"}     | ${"£0.64"}                | ${"2"}         | ${true}
  `(
