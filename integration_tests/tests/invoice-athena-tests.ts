...
describe("\nExecute athena query to retrieve invoice data and validate that it matches invoice files in storage s3 bucket\n", () => {
  ...
  test("retrieved invoice details should matches with invoice data in s3 bucket ", async () => {
    ...
    const s3Array:BillingStandardised[] = await s3GetObjectsToArray(testObject.bucket, folderPrefix);
     for (let i = 0; i < s3Array.length; i++) {
      expect(s3Array[i].invoice_receipt_id).toEqual(
        queryObjectsVal[i].invoice_receipt_id
      );
      expect(s3Array[i].vendor_name).toEqual(
        queryObjectsVal[i].vendor_name
      );
      expect(s3Array[i].total.toFixed(4)).toEqual(
        queryObjectsVal[i].total
      );
      expect(s3Array[i].invoice_receipt_date).toEqual(
        queryObjectsVal[i].invoice_receipt_date
      );
      expect(s3Array[i].subtotal.toFixed(4)).toEqual(
        queryObjectsVal[i].subtotal
      );
      expect(s3Array[i].due_date).toEqual(queryObjectsVal[i].due_date);
      expect(s3Array[i].tax.toFixed(2)).toEqual(queryObjectsVal[i].tax);
      expect(s3Array[i].tax_payer_id).toEqual(
        queryObjectsVal[i].tax_payer_id
      );
      expect(s3Array[i].item_id.toString()).toEqual(queryObjectsVal[i].item_id);
      expect(s3Array[i].item_description).toEqual(
        queryObjectsVal[i].item_description
      );
      expect(s3Array[i].service_name).toEqual(
        queryObjectsVal[i].service_name
      );
      expect(s3Array[i].unit_price.toFixed(4)).toEqual(
        queryObjectsVal[i].unit_price
      );
      expect(s3Array[i].quantity.toString()).toEqual(queryObjectsVal[i].quantity);
      expect(s3Array[i].price.toFixed(4)).toEqual(
